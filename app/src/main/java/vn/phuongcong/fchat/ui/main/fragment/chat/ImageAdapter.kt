 ...
 */
class ImageAdapter(var mListImage: MutableList<String>, var mContext: Context) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {


    override fun onBindViewHolder(holder: RecyclerView.ViewHolder?, position: Int) {
        if (holder is ImageViewHolder) {
            var pathImage = mListImage.get(position)
            holder.bin(pathImage, mContext)
        }
    }
    ...
    class ImageViewHolder(itemView: View?) : RecyclerView.ViewHolder(itemView) {
        ...
        fun bin(pathImage: String, mContext: Context) {
